...
class NPUModelRunner(GPUModelRunner):
    ...
    @contextmanager
    def _maybe_profile_run_mm_lora_contexts(self):
        """Prepare dummy tower/connector LoRA metadata for MM profile_run.

        This is a downstream Ascend-side guard for the upstream profile_run()
        multimodal encoder path. It does not patch GPUModelRunner. It only
        prepares TOWER / CONNECTOR dummy mappings before calling super().
        """
        ...
        if (
            ...
            or not hasattr(self, "lora_manager")
            or self.lora_manager is None
            or not self.lora_manager.supports_tower_connector_lora()
        ...
        if mm_budget is None or mm_budget.get_encoder_budget() <= 0:
            yield
            return
        ...
        if tower_token_counts and hasattr(self.model, "get_num_mm_connector_tokens"):
    ...
    def profile_run(self) -> None:
        ...
        with self._maybe_profile_run_mm_lora_contexts():
            super().profile_run()
